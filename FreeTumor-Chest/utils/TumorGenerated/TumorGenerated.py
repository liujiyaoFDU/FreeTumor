...
class TumorGenerated(RandomizableTransform, MapTransform):
    ...
    allow_missing_keys: bool = False
        ...
        #         texture = get_predefined_texture(predefined_texture_shape, sigma_a, sigma_b)
        #         self.textures.append(texture)
        self.textures = np.load('/home/linshan/FreeTumor/textures.npy')
    ...
    def __call__(self, data: Mapping[Hashable, NdarrayOrTensor]) -> Dict[Hashable, NdarrayOrTensor]:
        ...
        d['original_image'] = d['image'].clone()

        cls_list = list(torch.unique(d['label']))

        if self._do_transform and (1 in cls_list):
